                ...
                <div className="px-4 pt-3 pb-4 flex flex-col gap-3 overflow-hidden flex-1 min-h-0">
                    ...
                    <div className="flex flex-col gap-2 flex-shrink-0">
                        <div className="relative group flex-shrink-0">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-teal-500 transition-colors" size={18} />
                            <Input
                        ...
                        {suggestions.length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5">
                                <span className="text-[10px] font-bold text-stone-400 uppercase tracking-wider self-center">Suggestions :</span>
                                {suggestions.map((s) => (
                                    <button
                                        key={s}
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-12 gap-3">
                                <div className="relative">
                                    ...
                                    <ImageIcon size={18} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-teal-500" />
                                </div>
                            ...
                            <div className="p-6 text-center bg-red-50 rounded-xl border border-red-100">
                                ...
                                <Button variant="outline" onClick={() => handleSearch(query, source)} className="mt-3 border-red-200 text-red-600 hover:bg-red-100 hover:text-red-700 text-sm">
                                    Réessayer
                                </Button>
                            </div>
                        ) : results.length > 0 ? (
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                                {results.map((photo) => (
                                    <div
                                        key={photo.id}
                                        className="group relative aspect-[3/2] rounded-lg overflow-hidden bg-stone-100 cursor-pointer border border-stone-200 hover:border-teal-400 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
                            ...
                            <div className="py-12 text-center">
                                ...
                                <p className="text-stone-500 text-xs mt-1">Essayez d&apos;autres mots-clés ou suggestions.</p>
                            </div>
                        ) : (
                            <div className="py-12 text-center">
                                ...
                                <p className="text-stone-500 text-xs mt-1">Saisissez un mot-clé ou une suggestion.</p>
                            </div>
                        )}
                    </div>
                </div>
