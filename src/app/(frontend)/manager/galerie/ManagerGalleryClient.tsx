...
import { Search, Plus, Image as ImageIcon, Tag, FolderOpen, Trash2, Edit2, X } from 'lucide-react'
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {images.length === 0 ? (
        <Card className="flex flex-col items-center justify-center border-dashed py-16">
          ...
          <p className="mt-2 font-medium text-muted-foreground">Aucune image dans la galerie.</p>
          <Button variant="outline" className="mt-4" onClick={() => handleOpenImageSheet(null)}>
            Ajouter une image
          </Button>
        </Card>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {images.map((item) => {
            ...
            const tagList = Array.isArray(item.tags)
              ? item.tags.map((t) => (typeof t === 'object' && t ? (t as GalleryTag).name : null))
              : []
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Cette action est irréversible. L&apos;entrée de galerie sera supprimée (l&apos;image
              média reste en base).
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              disabled={isPending}
              onClick={() => deleteConfirm !== null && handleDeleteImage(deleteConfirm)}
            >
    ...
    <Sheet open={isOpen} onOpenChange={handleOpen}>
      <SheetContent className="flex w-full flex-col border-l border-border/50 sm:max-w-lg">
        <SheetHeader>
          ...
          <SheetDescription>Titre, image média, légende, catégorie et tags.</SheetDescription>
        </SheetHeader>
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-y-auto">
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium uppercase text-muted-foreground">Titre *</label>
              <Input
                name="title"
                required
            ...
            <div className="space-y-2">
              ...
              {!image && (
                <p className="text-xs text-muted-foreground">
                  Les médias sont ceux déjà uploadés (admin Payload ou autres écrans).
                </p>
              )}
            </div>

            <div className="space-y-2">
              <label className="text-xs font-medium uppercase text-muted-foreground">Légende</label>
              <textarea
                name="caption"
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium uppercase text-muted-foreground">Ordre</label>
              <Input name="order" type="number" defaultValue={image?.order ?? 0} className="w-24" />
            </div>
          </div>
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="flex w-full flex-col sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>Créez et modifiez les catégories de la galerie.</SheetDescription>
        </SheetHeader>
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="flex w-full flex-col sm:max-w-md">
        <SheetHeader>
          ...
          <SheetDescription>Créez et modifiez les tags de la galerie.</SheetDescription>
        </SheetHeader>
        <form onSubmit={handleCreate} className="flex gap-2 border-b border-border/50 pb-4">
          <Input name="newName" placeholder="Nouveau tag" className="flex-1" disabled={isPending} />
          <Button type="submit" disabled={isPending}>
            Ajouter
          </Button>
        </form>
