...
export async function deletePostcard(id: number): Promise<{ success: boolean; error?: string }> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    ...
    // Delete related view events
    await payload.delete({
      collection: 'postcard-view-events',
      where: { postcard: { equals: id } },
    })

    // Delete related tracking links
    await payload.delete({
    ...
    await payload.update({
      collection: 'leads',
      where: { usedByPostcard: { equals: id } },
      data: { usedByPostcard: null },
    })
  ...
  try {
    ...
    if (usersResult.totalDocs === 0) {
      return {
        success: false,
        error:
          'Aucun utilisateur agence associé à cette agence. Créez un utilisateur avec le rôle Agence lié à cette agence.',
      }
    }
...
export async function getAllGalleryCategories(params?: {
  page?: number
  limit?: number
  search?: string
}): Promise<GalleryCategoriesResult> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    const where: Where = {}
...
export async function createGalleryCategory(data: {
  ...
  slug?: string
  description?: string
}): Promise<{ success: boolean; data?: GalleryCategory; error?: string }> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    const result = await payload.create({
...
export async function getAllGalleryTags(params?: {
  page?: number
  limit?: number
  search?: string
}): Promise<GalleryTagsResult> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    const where: Where = {}
...
export async function createGalleryTag(data: {
  name: string
  slug?: string
}): Promise<{ success: boolean; data?: GalleryTag; error?: string }> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    const result = await payload.create({
...
export async function deleteGalleryTag(
  id: number | string,
): Promise<{ success: boolean; error?: string }> {
  await requireAdmin()
  try {
...
export async function createGalleryImage(
  data: Partial<Gallery> & { title: string; image: number },
): Promise<{ success: boolean; data?: Gallery; error?: string }> {
  await requireAdmin()
  try {
    const payload = await getPayload({ config })
    const result = await payload.create({
...
export async function getMediaForGallery(params?: { limit?: number; search?: string }): Promise<{
  ...
  try {
    ...
    const docs = (
      result.docs as { id: number; alt?: string; url?: string | null; filename?: string | null }[]
    ).map((d) => ({
      id: d.id,
      alt: d.alt ?? '',
      url: d.url ?? null,
...
export async function listR2Objects(prefix?: string, maxKeys = 100): Promise<ListR2Result> {
  ...
  try {
    ...
    const prefixes: string[] = (out.CommonPrefixes ?? []).map((p) =>
      (p.Prefix ?? '').replace(/\/$/, ''),
    )
    return {
      objects,
