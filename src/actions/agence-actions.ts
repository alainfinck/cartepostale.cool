...
async function requireAgence(): Promise<{ userId: number; agencyId: number } | null> {
  ...
  // For admins without agency, return null or a specific flag?
  // Let's return null if no agency even for admin,
  // because most functions below NEED an agencyId.
...
async function ensureAgencyOwnership(
  postcardId: number,
  agencyId: number,
  ...
  const docAgencyId =
    typeof doc.agency === 'object' && doc.agency ? (doc.agency as { id: number }).id : doc.agency
...
export async function getAgencyStats(): Promise<AgencyStats> {
  ...
  try {
    ...
    const [all, published, draft, archived] = await Promise.all([
      ...
      payload.find({
        collection: 'postcards',
        where: { ...base, status: { equals: 'published' } },
        limit: 0,
        depth: 0,
      }),
      payload.find({
        collection: 'postcards',
        where: { ...base, status: { equals: 'draft' } },
        limit: 0,
        depth: 0,
      }),
      payload.find({
  ...
  try {
    ...
    return {
      ...
      byCountry: Object.entries(byCountryMap)
        ...
        .slice(0, 10),
      byBrowser: Object.entries(byBrowserMap)
...
export async function getAgencyPostcardViewStats(
  postcardId: number,
): Promise<PostcardViewStats | null> {
  const auth = await requireAgence()
  if (!auth) return null
  const { agencyId } = auth
...
export async function updateAgencyPostcard(
  ...
  try {
    ...
    if (data.frontImageKey && typeof data.frontImageKey === 'string') {
      ...
      (data.frontImage as string).startsWith('data:image')
    ) {
...
export async function deleteAgencyPostcard(
  id: number,
): Promise<{ success: boolean; error?: string }> {
  const auth = await requireAgence()
  if (!auth) return { success: false, error: 'Non autorisé' }
